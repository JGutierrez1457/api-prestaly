module.exports = function(spenders, beneficiaries, quantity, sub_balanceInitial, own_products, exclude_products, existOwnProducts, existExcludeProducts, totalBeneficiaries ){
    return sub_balanceInitial.map( involved =>{//_id:username, amount:0 of involved
        ...
        if(existOwnProducts){
            ...
            if(hasNonOwnProducts){
                const noOwnProducts = own_products.filter( own_product => own_product.username !== involved._id );
                const pricesProducts = noOwnProducts.map( p => p.products.map( item => item.price-item.discount))
